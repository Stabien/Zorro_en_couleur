...
export const useStore = defineStore('main', {
      ...
      if (productsId.length > 0) {
        ...
        for (let i = 1; i < 5; i++) {
          ...
          for (let j = 0; j < productsId.length; j++) {
            if (currentCategory.find((item) => item.productId == productsId[j]) === undefined) j = productsId.length
            if (j == productsId.length - 1) categoryId.push(i)
          }
        }
        for (let i in categoryId) {
          for (let j in this.cloths) {
            if (this.cloths[j].categoryId == categoryId[i]) this.gallery.clothsToDisplay.push(this.cloths[j])
          }
        }
      } else {
  ...
  getters: {
    ...
    getOneClothById(state) {
      return (clothId) => {
        for (const cloth of state.cloths) if (cloth.id === clothId) return cloth
        return {}
      }
    },
    getProductByUuid(state) {
      return (uuid) => {
        for (const product of state.products) if (product.uuid === uuid) return product
        return {}
      }
    },
  },
